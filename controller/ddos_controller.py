...
Ryu OpenFlow 1.3 controller. No dependency on calibration_flows.csv or
CICDDoS2019_subset.csv -- those only feed the OFFLINE threshold calibration
in detection/entropy_detector.py. This controller computes entropy live,
from real flow-stats replies, using the threshold that calibration produced.

Responsibilities:
  1. L2 learning-switch forwarding (packet_in -> learn MAC -> flow_mod)
  2. Every POLL_INTERVAL seconds: polls flow stats, computes Shannon entropy
     of source IPs, and logs a metrics row (entropy, detection_flag,
     cpu_pct, mem_mb, n_flows, detection_latency_ms)
  3. Controller CPU/memory sampled via psutil every 500ms in a background
     thread (proposal Section 3.4: "sampled at 500ms intervals using psutil")
  ...
  5. Mitigation supports two modes (proposal Section 3.2 Step 5):
     MITIGATION_MODE = "drop" installs a DROP rule; "rate_limit" installs
     an OpenFlow 1.3 meter and attaches it to the flow instead.
...
POLL_INTERVAL = 2                 # seconds between flow-stats polls (1s/2s/5s tested in Phase 5)
ENTROPY_THRESH = 2.5              # bits -- calibrated via detection/entropy_detector.py --sweep
DROP_IDLE_TIMEOUT = 60            # seconds -- DROP/meter rule auto-expires
RESOURCE_SAMPLE_INTERVAL = 0.5    # seconds -- psutil sampling interval (proposal: 500ms)
MITIGATION_MODE = "drop"          # "drop" or "rate_limit"
RATE_LIMIT_KBPS = 100             # meter rate if MITIGATION_MODE == "rate_limit"
...
class DDoSController(app_manager.RyuApp):
    ...
    def _resource_monitor(self):
        self.proc.cpu_percent(interval=None)  # prime it once
        while True:
    ...
    def flow_stats_reply_handler(self, ev):
        ...
        for stat in ev.msg.body:
            ...
            if src_ip:
                src_ip_counter[src_ip] += stat.packet_count
        ...
        if detection_flag:
            ...
            mitigate_done = time.time()
            latency_ms = round((mitigate_done - self.entropy_below_thresh_since) * 1000, 2)
        else:
            self.entropy_below_thresh_since = None
